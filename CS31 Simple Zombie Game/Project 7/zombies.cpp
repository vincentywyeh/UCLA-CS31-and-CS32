...
private:
    ...
    int    m_row;
    int    m_col;
    //stores number of times zombie gets clubbed
    int    m_club;
};
...
void Zombie::move()
{
    ...
    //make row or column of zombie change as randomly chosen by dir
    m_arena -> determineNewPosition(m_row, m_col, dir);
}

bool Zombie::getAttacked(int dir)  // return true if dies
{
    //since second attack kills the zombie, if it has already been hit, it dies
    if (m_club > 0){
        return true;
    }else{
        //zombie got hit once
        ...
        switch(dir){
            case UP:
                //if there's space for moving up, row decreases by one, and if there isn't, the zombie dies
                if (m_row > 1){
                    m_row--;
                    return false;
                }
                ...
                break;
            case DOWN:
                //if there's space for moving down, row increases by one, and if there isn't, the zombie dies
                ...
                break;
            case LEFT:
                //if there's space for moving left, column decreases by one, and if there isn't, the zombie dies
                if (m_col > 1){
                    m_col--;
                    return false;
                }
                ...
                break;
            case RIGHT:
                //if there's space for moving right, column increases by one, and if there isn't, the zombie dies
...
{
    //player moves by one (or stays)
    m_age++;
    switch (dir) {
        case UP:
            //if player wants to move up and there're zombies above the player, attack zombie, and if there isn't, just move up
            ...
            break;
        case DOWN:
            //if player wants to move down and there're zombies below the player, attack zombie, and if there isn't, just move down
            ...
            break;
        case LEFT:
            //if player wants to move left and there're zombies on the left side of the player, attack zombie, and if there isn't, just move left
            ...
            break;
        case RIGHT:
            //if player wants to move right and there're zombies on the right side of the player, attack the zombie, and if there isn't, just move right
...
{
    //counts the number of zombies at a particular position
    int countZomb = 0;
    for (int i =0; i < m_nZombies; i++){
...
bool Arena::determineNewPosition(int& r, int& c, int dir) const
{
    ...
    {
        case UP:
            //if there's no space to move up, then don't move, and if there is, row should be decreased by one
            if (r == 1){
                return false;
            }else{
                r--;
            ...
            break;
        case DOWN:
            //if there's no space to move down, then don't move, and if there is, row should be increased by one
            if (r == m_rows){
                return false;
            }else{
                r++;
            ...
            break;
        case LEFT:
            //if there's no space to move left, then don't move, and if there is, column should be decreased by one
            if (c == 1){
                return false;
            }else{
                c--;
            ...
            break;
        case RIGHT:
              //if there's no space to move right, then don't move, and if there is, column should be increased by one
            if (c == m_cols){
                return false;
            }else{
                c++;
...
void Arena::display() const
{
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++){
            ...
            if (numZombiesAt(r+1, c+1) > 1 && numZombiesAt(r+1, c+1) < 9){
                grid[r][c] = numZombiesAt(r+1,c+1) + '0';
            }
...
{
    for (int i = 0; i < m_nZombies; i++){
        //finding the first zombie in the array whose position is the same as destinated and then break out of the loop
        if (m_zombies[i] -> row() == r && m_zombies[i] -> col() == c){
            //attack the zombie, and if the zombie dies, delete the zombie and "shift" the elements after the zombie to the left
...
bool Arena::moveZombies()
{
    ...
    {
        //move every zombies in one random direction and if the zombies hit the position of the player, the player dies
        m_zombies[k] -> move();
